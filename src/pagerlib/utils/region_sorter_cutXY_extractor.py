...
from pagerlib.dtypes import ImageSegment, Page
...
class RegionSorterCutXYExtractor:
    def __call__(self, page:Page):
        ...
        if len(text_regions) <= 1:
            return
        bboxes = [reg.segment for reg in text_regions]
        for i, bbox in enumerate(bboxes):
            bbox.id = i
        order = self.sort_using_XYCut(bboxes)
        page.children = no_text_regions+[text_regions[i] for i in order]

    @staticmethod
    def sort_using_XYCut(bboxes) -> list[int]:
